...
def parts():
    if 'cart' not in session:
        session['cart'] = {}
    if request.method == 'POST':
        for part in available_parts.keys():
            quantity = int(request.form.get(f'quantity_{part}', 0))
            if quantity > 0:
                session['cart'][part] = session['cart'].get(part, 0) + quantity
        session.modified = True
        return redirect(url_for('view_cart'))
    
    iphone_model = session.get('model', 'Unknown')
    return render_template('parts.html', parts=available_parts, prices=prices, cart=session['cart'], iphone_model=iphone_model)
...
def add_all_to_cart():
    ...
    for part in parts:
        if part.startswith('quantity_'):
            ...
            if part_name in parts_with_colors:
                colors = request.form.getlist(f'color_{part_name}')
            else:
            ...
            for quantity, color in zip(quantities, colors):
                if int(quantity) > 0:
                    ...
                    if color != 'none':
                        item['color'] = color
                    session['cart'][item_key] = item

                    # Save to database
                    cursor = db.cursor()
                    cursor.execute(
                        "INSERT INTO user_cart (user_id, part_name, quantity, color) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE quantity=%s",
                        (user_id, part_name, int(quantity), color, int(quantity))
                    )
                    db.commit()
                    cursor.close()
...
def view_cart():
    ...
    total_price = sum(item.get('price', 0) * item.get('quantity', 0) for item in session['cart'].values())
    print(f"Cart Items: {session['cart']}")  # Debug statement
    ...
    if request.method == 'POST':
        ...
        if 'otp' in session and int(entered_otp) == session['otp']:
            ...
            for item_key, item in session['cart'].items():
                part_name = item_key.split('_')[0]  # Extract the part name
            ...
            iphone_model = session.get('model', 'Unknown') # Ensure iphone_model is defined

            try:
                cursor = db.cursor()
                cursor.execute("""
                    INSERT INTO orders (order_id, user_name, email, phone, address, iphone_model, total_price, order_details)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """, (order_id, user_name, email, phone, address, iphone_model, total_price, order_details_str))
                db.commit()
                cursor.close()
            ...
            return render_template('checkout.html', error=error, cart=session['cart'], total_price=sum(prices.get(part.split('_')[0], 0) * item['quantity'] for part, item in session['cart'].items()))
